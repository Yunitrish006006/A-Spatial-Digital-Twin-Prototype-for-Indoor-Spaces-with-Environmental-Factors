...
\begin{abstract}
Many room-scale digital twins assume that influential appliances are network-connected and can report their states directly. In ordinary rooms, however, conventional air conditioners, manual windows, and ordinary lights often expose no telemetry, while only a few sensors can be installed. This paper presents a single-room spatial digital twin developed under these constraints. The final design combines an appliance-aware reduced-order bulk-plus-local field model, eight-corner residual calibration, least-squares appliance-impact learning, and an optional hybrid residual neural layer rather than an end-to-end black-box field predictor. The prototype is exposed through a local Model Context Protocol (MCP) server and a rotatable 3D web demo. Across eight canonical scenarios, the base model achieves average MAE of 0.0482 for temperature, 0.1763 for humidity, and 2.1616 for illuminance; on held-out synthetic scenarios, the hybrid residual layer further reduces field MAE from 0.0473/0.1764/2.3727 to 0.0026/0.0043/0.2357. These results indicate that sparse corner sensing can still support an interpretable and trainable indoor twin when model structure, calibration, and learning are assigned to different layers.
\end{abstract}
...
\begin{itemize}
\item A single-room three-factor digital twin that combines a bulk-plus-local field model with appliance-aware influence functions under a fixed eight-corner sensing layout.
...
\item A data pathway that keeps the reduced-order model as the primary estimator and uses neural learning only for residual correction, together with a task-aligned strategy for comparing the prototype against partially compatible public datasets.
\item A deployable research prototype exposed through MCP tools and a local 3-D web interface, evaluated on canonical synthetic scenarios, a window condition matrix, and held-out residual-learning cases.
\end{itemize}
...
Under the default held-out split used in the current prototype, six scenarios are used for residual-network training and two scenarios (\texttt{light\_only} and \texttt{all\_active}) are held out for evaluation. The average field MAE is reduced from 0.0473 to 0.0026 for temperature, from 0.1764 to 0.0043 for humidity, and from 2.3727 to 0.2357 for illuminance. These reductions suggest that a residual neural layer can absorb structured error left by the reduced-order model while preserving the base model's interpretability.
...
Not every public indoor-environment dataset can be used as a direct benchmark for the full prototype. Full-field reconstruction requires dense room-scale ground truth, which most public datasets do not provide. A fairer strategy is task-aligned comparison. For example, CU-BEMS can support zone-level air-conditioning and lighting response analysis because it contains indoor environmental measurements together with power series \cite{chitalia2020cubems}. SML2010 is more suitable for two-point daylight and window-response prediction because it provides indoor temperature, humidity, illuminance, and outdoor weather signals at sparse locations \cite{romeu2014sml2010}. Room-level residential IEQ datasets such as the Denmark apartment dataset can support occupancy-aware room response analysis without claiming full-field supervision \cite{andersen2024denmarkieq}. Large comfort corpora such as the ASHRAE Global Thermal Comfort Database II are more appropriate for validating comfort-oriented scoring or action objectives than for spatial reconstruction \cite{licina2018ashraeglobal}. In other words, canonical synthetic scenarios should remain the main benchmark for full spatial-field evaluation, while public datasets should be used for the subproblems they actually support.

The prototype is intentionally lightweight: it does not attempt CFD-grade airflow modeling, full wall thermal mass modeling, or detailed optical rendering. The humidity component is modeled using a simplified first-order coupling with the air conditioner and window, so humidity should be interpreted as secondary evidence compared with temperature and illuminance. In addition, the current evaluation uses simulated observations with deterministic perturbations rather than long-term real deployment data. The hybrid residual layer improves held-out reconstruction accuracy in the current setting, but its reported gain should still be interpreted as an extension built on synthetic truth rather than a replacement for real-room validation.

\section{Conclusion}
This paper presented an MCP-enabled spatial digital twin prototype for learning environmental impacts of non-networked appliances in a single room. The core result is not merely that a room can be simulated, but that sparse corner sensing, appliance-aware modeling, calibration, and residual learning can be assigned complementary roles within one coherent system. The prototype estimates temperature, humidity, and illuminance fields, learns appliance impacts from before-and-after observations, ranks candidate control actions, and exposes the resulting capabilities through MCP tools and an interactive 3D web demo. The reported results show that this layered design is practical under sparse sensing constraints and that hybrid residual learning is most useful when it refines, rather than replaces, the reduced-order twin. Future work will focus on physical ESP32 deployment, task-aligned public-dataset benchmarking, retraining on measured data, remote MCP deployment, and eventually closed-loop control.
