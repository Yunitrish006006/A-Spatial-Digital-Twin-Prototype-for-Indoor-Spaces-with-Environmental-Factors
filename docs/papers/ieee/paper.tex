...
Under the default held-out split used in the current prototype, six scenarios are used for residual-network training and two scenarios (\texttt{light\_only} and \texttt{all\_active}) are held out for evaluation. With Fourier low-pass denoising enabled for temperature and humidity residual traces only, the average field MAE is reduced from 0.0473 to 0.0026 for temperature, from 0.1764 to 0.0040 for humidity, and from 2.3727 to 0.2357 for illuminance. These reductions suggest that a residual neural layer can absorb structured error left by the reduced-order model while preserving the base model's interpretability, and that frequency-domain denoising is beneficial mainly for the slower thermal and humidity responses.
...
\section{Conclusion}
This paper presented a sparse-sensing spatial digital twin for learning environmental impacts of non-networked appliances in a single room. The core result is not merely that a room can be simulated, but that sparse corner sensing, appliance-aware modeling, calibration, and residual learning can be assigned complementary roles within one coherent system. The prototype estimates temperature, humidity, and illuminance fields, learns appliance impacts from before-and-after observations, ranks candidate control actions, and exposes the resulting capabilities through a local service layer and an interactive 3D web demo. The reported results show that this layered design is practical under sparse sensing constraints and that hybrid residual learning is most useful when it refines, rather than replaces, the reduced-order twin. Future work will focus on physical ESP32 deployment, task-aligned public-dataset benchmarking, retraining on measured data, broader service deployment, and eventually closed-loop control.
