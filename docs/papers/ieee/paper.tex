...
\item A deployable research prototype with interactive service interfaces, including a local MCP endpoint and a 3-D web interface, evaluated on canonical synthetic scenarios, ablation variants, a leave-one-scenario-out residual benchmark, a window condition matrix, and task-aligned public datasets.
\end{itemize}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../thesis/assets/fig_3_1_overall_architecture.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../thesis/assets/fig_3_3_room_topology.png}
...
All full-field experiments use the same 6.0\,m by 4.0\,m by 3.0\,m room, three appliance locations, three evaluation zones, and eight corner sensors shown in Fig.~\ref{fig:room_topology}. The canonical grid resolution is $16\times 12\times 6$, yielding 1152 spatial points per field. Unless otherwise stated, scenarios are evaluated after an 18-minute settling interval under an outdoor boundary condition of 33.0$^\circ$C, 74.0\% relative humidity, and 32000 lux sunlight. The validation set contains eight appliance-state combinations: idle, AC only, window only, light only, AC+window, window+light, AC+light, and all active.

Because this submission does not yet include a physical room deployment, the dense field target is generated by applying deterministic truth adjustments to appliance power scales and then synthesizing corner observations from the truth sensor predictions. The observation perturbation is fixed by sensor index: temperature adds $0.08((i\bmod 4)-1.5)$, humidity adds $0.3((i\bmod 4)-1.5)$, and illuminance adds $3.0((i\bmod 4)-1.5)$. This makes every result exactly reproducible and keeps the synthetic supervision separate from the nominal estimator. The main scripts used for reproduction are \texttt{scripts/run\_demo.py}, \texttt{scripts/run\_hybrid\_residual\_experiment.py}, \texttt{scripts/run\_submission\_readiness\_experiments.py}, and \texttt{scripts/run\_public\_dataset\_model\_comparison.py}. The residual network uses one hidden layer with 10 units for the leave-one-scenario-out benchmark, L2 regularization of $10^{-5}$, 80 epochs, a learning rate of 0.018, and a fixed random seed.
...
Across the eight validation scenarios, the average mean absolute error (MAE) was 0.0474 for temperature, 0.1765 for humidity, and 2.0835 for illuminance. Temperature and humidity errors are small relative to their physical scales, while illuminance remains more difficult because it depends strongly on source position, daylight, obstruction, and indirect reflection. Fig.~\ref{fig:field_mae_comparison} compares the proposed base estimator, IDW, and the leave-one-scenario-out hybrid estimator. The large IDW illuminance error confirms that geometry-free interpolation is inadequate when active light and window sources dominate the field.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{assets/field_mae_comparison.png}
\caption{Field MAE comparison among IDW, the base estimator, and leave-one-scenario-out hybrid residual correction. Values are averaged over the eight canonical scenarios.}
\label{fig:field_mae_comparison}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../thesis/assets/fig_5_7_light_only_illum_3d.png}
...
\hline
light\_only & Temp. & 0.0470 & 0.1232 & 61.85\%\\
light\_only & Hum. & 0.1762 & 0.4656 & 62.16\%\\
light\_only & Illum. & 2.5774 & 112.2559 & 97.70\%\\
...
\subsection{Synthetic Ablation Study}
To make the synthetic evidence more defensible, Table~\ref{tab:ablation} reports ablations on the same eight scenarios, sensor layout, grid, and deterministic truth generation. Removing appliance-aware feedback entirely produces larger temperature and illuminance errors. Removing the one-bounce reflection term mainly affects illuminance, while removing active-device power calibration also increases illuminance error. The no-trilinear variant gives lower dense-field error for humidity and illuminance in this synthetic benchmark because it calibrates active device power but does not force the full interior field to pass through a corner-fitted residual surface. This result is useful rather than contradictory: it shows that trilinear correction should be interpreted as a sparse-sensor consistency layer, not as a guarantee of lower dense-field MAE under every synthetic truth.

\begin{table}[t]
\caption{Average Field MAE Under Synthetic Ablations}
\label{tab:ablation}
\centering
\scriptsize
\begin{tabular}{|l|c|c|c|}
\hline
Variant & Temp. & Hum. & Illum.\\
\hline
...
Under the default held-out split used in the current prototype, six scenarios are used for residual-network training and two scenarios (\texttt{light\_only} and \texttt{all\_active}) are held out for evaluation. This split contains 576 training samples and 192 test samples. Table~\ref{tab:hybrid} reports both this split and a leave-one-scenario-out (LOO) evaluation. With Fourier low-pass denoising enabled for temperature and humidity residual traces only, the default split reduces field MAE from 0.0474/0.1765/2.3087 to 0.0023/0.0041/0.1675. Disabling Fourier denoising gives 0.0022/0.0045/0.1675, showing that denoising mainly stabilizes humidity and is not responsible for the illuminance gain. In LOO evaluation, each fold trains on seven scenarios and tests on the remaining scenario, with 672 training samples and 96 test samples per fold. The average LOO hybrid MAE is 0.0017/0.0055/0.1581, corresponding to 96.41\%, 96.88\%, and 92.41\% reductions relative to the base estimator.

\begin{table}[t]
\caption{Hybrid Residual Robustness Checks (Temp./Hum./Illum.)}
\label{tab:hybrid}
\centering
\scriptsize
\begin{tabular}{|l|c|c|c|}
\hline
Setting & Samples & Base MAE & Hybrid MAE\\
\hline
Default 6/2 & 576/192 & .0474/.1765/2.3087 & .0023/.0041/.1675\\
No Fourier & 576/192 & .0474/.1765/2.3087 & .0022/.0045/.1675\\
LOO avg. & 672/96 & .0474/.1765/2.0835 & .0017/.0055/.1581\\
\hline
\end{tabular}
\end{table}
