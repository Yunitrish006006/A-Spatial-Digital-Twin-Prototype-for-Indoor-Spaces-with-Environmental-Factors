...
\hline
7. Candidate action simulation & 目前註冊設備與候選動作，例如開冷氣、調整冷氣模式、開窗或開燈。 & 只在 `READY\_\allowbreak{}TO\_\allowbreak{}RANK` 時，對每個候選動作建立反事實 scenario，把裝置 activation 或 metadata 改成候選狀態，重新執行同一條推論流程。 & 每個候選動作後的 $\mathbf{q}_a=(q_T,q_H,q_L)$。 \\
\hline
