...
圖 5-7 僅開窗溫度場 3D 點雲（window\_\allowbreak{}only）…… 18


圖 5-8 僅燈具照度場 3D 點雲（light\_\allowbreak{}only，5.11 節）…… 19
...
\item RQ3：學習後的裝置影響模型，是否能依三因子偏差輸出候選控制動作排序，例如選擇開冷氣、開窗或開燈，且推薦動作的實際可行性應如何透過介入式 before/after 實驗驗證？
...
\hline
1. Runtime input & MCP initialize、web demo、script 或 API 傳入 baseline、外部環境、設備狀態、家具、elapsed/steady-state、查詢座標與目標值。 & 使用 `\_\allowbreak{}scenario\_\allowbreak{}with\_\allowbreak{}overrides` 或 MCP registered state 建立目前房間狀態。 & 可推論的 scenario state。 \\
\hline
...
\hline
5. Point / zone prediction & 指定座標或目標區域。 & 若是 `sample\_\allowbreak{}point`，直接回傳該點 temperature、humidity、illuminance；若是 zone summary，對區域內採樣點做平均或統計。 & 預測溫度、濕度、照度與 estimator 狀態。 \\
\hline
...
\item rank\_\allowbreak{}actions：輸入指定座標與目標三因子值，根據目前註冊設備產生候選操作並依 comfort penalty 改善量排序。
\end{itemize}

其中 initialize\_\allowbreak{}environment 是 MCP runtime 的起點，不是單純把場景名稱設為 idle。它會建立後續工具共用的 session state，因此必須清楚區分哪些項目是在初始化時註冊，哪些是後續查詢才輸入。表 4-2 列出目前初始化可設定的內容。


表 4-2 initialize\_\allowbreak{}environment 可設定內容
...
\hline
light\textbackslash{}\_\allowbreak{}only & 照度 & 2.5774 & 112.2559 & 97.7\% \\
\hline
window\textbackslash{}\_\allowbreak{}light & 照度 & 2.0276 & 151.2615 & 98.7\% \\
\hline
ac\textbackslash{}\_\allowbreak{}light & 照度 & 2.5008 & 105.2485 & 97.6\% \\
\hline
all\textbackslash{}\_\allowbreak{}active & 溫度 & 0.0479 & 0.1896 & 74.7\% \\
\hline
all\textbackslash{}\_\allowbreak{}active & 照度 & 2.0399 & 131.6092 & 98.5\% \\
\hline
\end{tabular}
\end{table}
...
\footnotesize
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.111\textwidth}|>{\raggedright\arraybackslash}p{0.111\textwidth}|>{\raggedright\arraybackslash}p{0.111\textwidth}|>{\raggedright\arraybackslash}p{0.111\textwidth}|>{\raggedright\arraybackslash}p{0.111\textwidth}|>{\raggedright\arraybackslash}p{0.111\textwidth}|>{\raggedright\arraybackslash}p{0.111\textwidth}|}
\hline
...
\subsubsection*{5.9.1 優勢分析}

第一項優勢是事件或邊界變化任務的趨勢追蹤能力。在 SML2010 S3 任務中，persistence 的 Correlation 值全為 0.000，原因是任務標的為 facade 條件變化後的 delta response，前一時步值無法提供變化方向。相較之下，本研究模型在 S3 15min dining\_\allowbreak{}temperature 的 Correlation 達 0.950，S3 15min dining\_\allowbreak{}humidity 達 0.612，S3 60min dining\_\allowbreak{}temperature 達 0.937，均為三者最高。這說明物理結構與設備影響函數提供的先驗資訊，在環境條件快速變化時比純時序延續有更強的趨勢預測能力。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.83\textwidth]{assets/fig_5_6_all_active_temp_3d.png}
\par\smallskip
{\small 圖 5-6 三裝置同時作用（all\textbackslash{}\_\allowbreak{}active）溫度場 3D 點雲。顏色由藍綠至橙紅對映 26.02–27.16 °C 範圍；設備位置用帶邊框的標記表示。}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.83\textwidth]{assets/fig_5_7_window_only_temp_3d.png}
\par\smallskip
{\small 圖 5-7 僅開窗（window\textbackslash{}\_\allowbreak{}only）溫度場 3D 點雲。靠窗區域溫度最高，造成區域溫度梯度。可與 all\textbackslash{}\_\allowbreak{}active 情境對比，觀察冷氣介入後對窗邊高溫的抑制效果。}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.83\textwidth]{assets/fig_5_8_light_only_illum_3d.png}
\par\smallskip
{\small 圖 5-8 僅燈具作用（light\textbackslash{}\_\allowbreak{}only）照度場 3D 點雲。燈具正下方照度最高，遠端補有少量的 single-bounce diffuse 回填；此情境也是 5.7 節 held-out 測試集之一。}
\end{figure}
...
\item 控制功能目前為推薦排序，尚未完成真實介入式因果驗證，也尚未進入自動閉環控制。
\end{itemize}
...
\item 依 before/after 介入驗證方法實測推薦動作，量化 actual improvement、success rate 與 top-1 regret。
