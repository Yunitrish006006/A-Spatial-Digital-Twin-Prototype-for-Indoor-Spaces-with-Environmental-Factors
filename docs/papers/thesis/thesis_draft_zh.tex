...
表 5-1 IDW 與本研究 field MAE 比較…… 14
...
表 5-4 Benchmark 層級設計…… 16


表 5-5 SML2010 Benchmark 結果…… 16


表 5-6 CU-BEMS Benchmark 結果…… 18
...
\par\smallskip
{\small 圖 3-1 系統整體分層架構。此圖說明 Web、MCP/Gemma 入口如何共用 service layer，並串接主模型與 hybrid residual layer。}
\end{figure}
...
\par\smallskip
{\small 圖 5-2 三裝置同時作用（all\textbackslash{}\_\allowbreak{}active）之溫度場 3D 點雲視觀圖。每點為一個 16×12×6 網格樣本，顏色由藍綠（低溫）至橙紅（高溫）映射溫度分布。冷氣區域明顯偏藍，靠窗與靠燈區域則居溫度中高端。}
\end{figure}
...
\par\smallskip
{\small 圖 5-3 僅冷氣作用（ac\textbackslash{}\_\allowbreak{}only）之溫度場 3D 點雲視觀圖。冷氣氣流影響區域（後牆靠所側）溫度明顯下降，距冷氣較遠的靠窗區域溫度相對較高，展示 bulk + local field 模型對全室平均與局部梯度的同時建模能力。}
\end{figure}
...
\par\smallskip
{\small 圖 5-4 僅燈具作用（light\textbackslash{}\_\allowbreak{}only）之照度場 3D 點雲視觀圖。燈具正下方的測點照度最高（黃橙色），底層四角與遠端則由間接反射賦予少量回填亮度，顏色實現正確的照度衰減樣態。}
\end{figure}
...
表 5-5 列出 SML2010 benchmark 各任務三項指標的完整對比。
...
表 5-6 列出 CU-BEMS benchmark 各任務三項指標的完整對比。
...
第二項優勢是長視窗溫度任務的 MAE 與 RMSE。在 60 分鐘預測視窗下，S2 dining\_\allowbreak{}temperature 的 MAE 降至 0.156、RMSE 為 0.202，S3 dining\_\allowbreak{}temperature MAE 為 0.190、RMSE 為 0.243，均為三者最低。persistence 在 60 分鐘後誤差急速累積（S2 persistence MAE 0.470），而本研究因為納入設備狀態與外部條件的結構性估計，使長視窗誤差增長較緩。


第三項優勢是複合任務的 RMSE 表現。S3 60min dining\_\allowbreak{}illuminance 的 RMSE 為 19.344，低於 persistence 的 21.931 與 linear regression 的 22.763；S3 60min room\_\allowbreak{}illuminance 的 RMSE 為 26.027，也低於另外兩者。RMSE 對離群誤差的放大效應更敏感，這表示本研究模型在極端照度事件的重建上較其他方法更為穩定。
...
第二個劣勢是 SML2010 濕度的系統性高估。S2 15min dining\_\allowbreak{}humidity MAE 為 0.832，遠高於 persistence 的 0.198 與 linear regression 的 0.209；S2 15min room\_\allowbreak{}humidity 同樣如此（0.703 對 0.154）。這反映本研究的濕度特徵對齊方式與 SML2010 的量測尺度存在系統性偏差，即模型濕度估計值的基準點偏移，而非隨機誤差。此問題在 S3 複合任務中得到部分改善（S3 dining\_\allowbreak{}humidity MAE 0.335，接近 persistence 0.386），推測是複合任務的外氣濕度特徵提供了更多修正訊號。
...
\par\smallskip
{\small 圖 5-6 僅開窗（window\textbackslash{}\_\allowbreak{}only）溫度場 3D 點雲。靠窗區域溫度最高，造成區域溫度梯度。可與 all\textbackslash{}\_\allowbreak{}active 情境對比，觀察冷氣相抑後的溫度減診效果。}
\end{figure}
...
\par\smallskip
{\small 圖 5-7 僅燈具作用（light\textbackslash{}\_\allowbreak{}only）照度場 3D 點雲。燈具正下方照度最高，遠端補有單的 single-bounce diffuse 回填；此情境也是 5.6 節 held-out 測試集之一。}
\end{figure}
