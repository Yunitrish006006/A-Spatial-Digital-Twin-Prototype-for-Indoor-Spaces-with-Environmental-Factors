...
本研究固定使用 8 顆角落感測器，因此目前採用 trilinear residual correction。相較於一階 affine correction，trilinear correction 能使用 8 個角點支撐 8 個校正係數：

```text
C_v(x, y, z) = c0 + c1*X + c2*Y + c3*Z + c4*X*Y + c5*X*Z + c6*Y*Z + c7*X*Y*Z
```

其中 `X/Y/Z` 是正規化後的房間座標。用 8 個感測器的觀測殘差估計 `c`：
...
- 當作較平滑的插值 baseline。
- 未來如果要進一步提高空間平滑度，可作為 trilinear correction 之外的比較模型。
...
- 作為文獻探討的重要模型。
- 作為未來改進版本。
- 若論文後期需要更強的方法論，可將 trilinear correction 升級或擴充為 Gaussian Process correction。
...
- 三因子場估計
- active-device power calibration
- 8 角落感測器 trilinear correction
- 目標區域平均值
- 候選設備動作排序

下一步若要加強研究深度，建議優先加入：

1. IDW baseline，與 trilinear correction 比較。
2. 不同外部環境條件的敏感度分析。
