...
具體而言，本研究可採兩層 benchmark 設計。第一層是 canonical synthetic benchmark，直接使用本研究的 8 組標準情境與 48 組窗戶矩陣，讓主模型、IDW baseline、移除設備先驗的純資料驅動模型，以及 hybrid residual correction 在完全相同的輸入、感測器配置與 ground truth 下比較 field MAE、zone MAE、sensor MAE 與推薦改善分數。第二層是 public task-aligned benchmark，亦即把公開資料集拆成與本研究相容的子任務：CU-BEMS 可用於比較 AC/lighting 事件前後的 zone-level temperature、humidity、illuminance 響應；SML2010 可用於比較窗戶/日照相關的溫濕度照度時序響應；Denmark IEQ 與 ASHRAE Global Thermal Comfort Database II 則適合比較舒適度目標函數、偏差分數或分類準確率。此種設計的好處在於，不必誇大公開資料集與本研究完全同構，仍能建立「同一資料、同一輸入、同一輸出、同一指標」的公平比較框架。

因此，答案並不是公開資料集完全不能拿來與本研究比較，而是需要把比較任務切分。若研究目標是完整 3D 空間場重建與非連網裝置影響學習，目前最公平的同資料集 benchmark 仍是本研究自建的可控制模擬資料；若研究目標是設備事件響應、窗戶日照響應或舒適度評分，則可以在 CU-BEMS、SML2010、Denmark IEQ 或 ASHRAE 資料上建立共享子任務 benchmark。這樣既能保留本研究的系統特色，也能回應口試委員對同資料集比較公平性的要求。

若將上述想法整理成可操作的 benchmark 矩陣，則可進一步表示如下。

| benchmark 層級 | 資料來源 | 比較任務 | 本研究模型模式 | 共同輸入 | 共同輸出 | 建議指標 |
| --- | --- | --- | --- | --- | --- | --- |
| canonical synthetic | 8 組標準情境 | 完整場重建、裝置影響學習、推薦排序 | full spatial mode | 設備狀態、8 顆角落感測器、室內外條件 | field、zone、sensor、ranking | field MAE、zone MAE、sensor MAE、improvement score |
| canonical synthetic | 48 組窗戶矩陣 | 外部邊界條件敏感度分析 | full spatial window mode | outdoor temperature、outdoor humidity、sunlight、opening ratio | window_zone、center_zone、door_side_zone | field MAE、zone deviation、趨勢一致性 |
| public task-aligned | CU-BEMS | AC 與照明事件後的區域響應 | single-zone dataset-compatible mode | timestamp、zone temperature、zone humidity、zone illuminance、AC kW、lighting kW、plug kW | zone-level temperature、humidity、illuminance | MAE、RMSE、delta MAE、correlation |
| public task-aligned | SML2010 | 兩點日照與外氣條件響應 | two-point dataset-compatible mode | DateTime、兩點 temperature/humidity/illuminance、outdoor temperature、outdoor humidity、facade sunlight、sun irradiance、rain、wind | dining-room 與 room 的 temperature、humidity、illuminance | MAE、RMSE、delta MAE |
| public task-aligned | Denmark IEQ / ASHRAE | 舒適度評分與控制目標合理性 | comfort-only mode | indoor environment、occupancy、subjective vote | comfort score、comfort class | score error、accuracy、F1、AUROC |

其中需要特別說明的是：只有 canonical synthetic benchmark 同時具備完整空間場 truth，因此 field MAE 與區域空間差異分析應主要在該層報告；公開資料集 benchmark 則應集中在 zone-level、point-level 或 comfort-level 的共享任務，避免把沒有空間 ground truth 的資料誤用為 3D 場重建基準。
...
![圖 3-1 系統整體分層架構。此圖說明 Web、MCP/Gemma 入口如何共用 service layer，並串接主模型與 hybrid residual layer。](../../outputs/figures/architecture/整體分層架構.svg)
...
```text
F_v(x, y, z, t) = B_v^bulk(t) + B_v^local(x, y, z, t) + Σ I_j,v^local(x, y, z, t) + C_v(x, y, z)
```
...
```text
F_v^hybrid(x, y, z, t) = F_v(x, y, z, t) + R_v(x, y, z, t; θ_v)
```

其中 `F_v` 為第三章前述的 reduced-order 主模型，`R_v` 則由小型多層感知器近似其殘差。訓練目標定義為：

```text
R_v*(x, y, z, t) = F_v^truth(x, y, z, t) - F_v(x, y, z, t)
```

其損失函數可表示為：

```text
L(θ_v) = (1 / N) Σ_i ||R_v*(p_i, t_i) - R_v(p_i, t_i; θ_v)||^2 + λ||θ_v||^2
```
...
為避免不同資料來源與不同任務被混為單一 benchmark，本章比較分為兩層。第一層是同一組標準模擬資料上的內部 benchmark，也就是在完全相同的情境、感測器配置與 truth field 下，直接比較本研究主模型、IDW baseline 與 hybrid residual correction。第二層則是與公開資料集可對齊的子任務比較，其原則是只在共享變數與共享輸出的前提下比較時序反應、外部條件響應或舒適度評分，而不宣稱所有方法都能在完整單房間 3D 空間場上做一對一對照。換言之，第五章的數值結果主要對應第一層 benchmark，而第二層 benchmark 則作為後續接入真實資料與外部資料集驗證的設計依據。
...
## 5.6 Hybrid Residual Neural Network 結果
...
綜合 SML2010 與 CU-BEMS 的比較結果，可歸納如下：
