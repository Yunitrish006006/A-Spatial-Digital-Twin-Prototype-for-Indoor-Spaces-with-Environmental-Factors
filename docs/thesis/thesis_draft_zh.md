...
第五章 模擬案例與結果分析…… 13
  ...
  5.8 公開資料集 Task-Aligned Benchmark 結果…… 16
...
表 5-4 Hybrid residual robustness checks…… 15

表 5-5 窗戶矩陣情境節選…… 15

表 5-6 SML2010 Benchmark 結果…… 16

表 5-7 CU-BEMS Benchmark 結果…… 17
...
圖 5-6 三裝置全開溫度場 3D 點雲（5.10 節）…… 18

圖 5-7 僅開窗溫度場 3D 點雲（window_only）…… 18

圖 5-8 僅燈具照度場 3D 點雲（light_only，5.10 節）…… 18
...
表 5-4 彙整預設切分、no-Fourier 對照與 LOO cross-validation。圖 5-5 則將 IDW、base model 與 LOO hybrid 的平均 field MAE 以 log-scale 顯示，避免照度量級過大而掩蓋溫度與濕度差異。
...
## 5.8 公開資料集 Task-Aligned Benchmark 結果
...
表 5-6 列出 SML2010 benchmark 各任務三項指標的完整對比。
...
表 5-7 列出 CU-BEMS benchmark 各任務三項指標的完整對比。
