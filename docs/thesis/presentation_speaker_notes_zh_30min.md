...
8 組標準情境中，base model 平均 field MAE 為 temperature 0.0474、humidity 0.1765、illuminance 2.0269。

圖中比較 IDW、base model 與 leave-one-scenario-out hybrid residual。IDW 因缺少裝置位置、方向與物理先驗，在照度與局部場上特別不利。

真實臥室 pillow 點的 raw MAE 為 T=0.8967, H=4.1286, L=309.0142，校正後 MAE 為 T=0.1676, H=0.3939, L=16.6450，顯示稀疏校正在此設定下有明顯改善。
...
右側重點包含：L_lightᵈⁱʳ(p,t) = G_light P_light A_light(t) Φ_light(p) Q_light(p) V_light(p)；Φ_light 是由光束角推得的 cosine 方向權重；Q_light 是參考距離正規化後的距離衰減；V_light 是燈具到查詢點的遮蔽或可見性；L_winᵃᵐᵇ(p,t)：窗戶帶來的擴散環境光。
...
- **光束角**：描述燈具主要出光範圍的角度；角度越窄，方向衰減越集中。
- **Cosine 方向權重**：用燈具朝向與查詢點方向的夾角投影近似光強分布。
- **距離衰減**：光源距離查詢點越遠，直接照度越低；本研究用參考距離正規化避免量級失控。
