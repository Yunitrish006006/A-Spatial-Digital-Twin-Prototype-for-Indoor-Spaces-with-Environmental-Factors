...
\begin{abstract}
Indoor digital twins are often designed under the assumption that appliances are network-connected and can directly report their operating states. In ordinary rooms, however, many devices and environmental elements remain non-networked: a conventional air conditioner may not expose its actual output, a manual window cannot report its opening degree, and ordinary lights may not provide feedback beyond environmental changes. This paper presents a lightweight spatial digital twin prototype that learns the environmental impact of non-networked appliances from limited room sensors. The proposed prototype models a single room as three spatial fields: temperature, humidity, and illuminance. Eight corner sensor nodes are used to calibrate a physics-inspired appliance influence model through affine residual correction. The system further estimates active appliance impact coefficients from before-and-after sensor observations and ranks candidate control actions by target-zone comfort improvement. To support AI-agent access, the prototype exposes its simulation, point estimation, baseline comparison, impact learning, and window-matrix analysis through a local Model Context Protocol (MCP) server. The implementation includes a Python simulation backend, an inverse distance weighting baseline, a 48-case window simulation matrix, and an interactive rotatable 3D web demo. Simulation results show that the proposed appliance-aware model can provide interpretable field reconstruction and action recommendations under sparse sensing constraints.
\end{abstract}
...
This paper addresses the following research question: can a single-room spatial digital twin estimate temperature, humidity, and illuminance distributions, learn the impact of non-networked appliances from limited corner sensors, and expose the resulting capabilities as AI-agent-accessible tools? We propose a lightweight prototype that uses eight fixed corner sensors, parameterized appliance influence functions, affine sensor residual correction, and before-and-after impact learning.
...
\item A simulation study including eight appliance-combination scenarios, an inverse distance weighting (IDW) baseline, and a 48-case window matrix across time of day, weather, and season.
\end{itemize}

\section{Related Work}
\subsection{Indoor Digital Twins and Environmental Modeling}
Digital twins provide computational representations of physical systems and are increasingly used in manufacturing, infrastructure, and smart buildings \cite{tao2019digitaltwin,fuller2020digitaltwin}. In buildings, a digital twin may integrate building geometry, sensors, equipment states, and control logic. However, many approaches assume access to either detailed building management systems or connected devices. This work focuses on a smaller but common scenario: a single room with limited sensors and appliances that may not report their states.

Indoor thermal and comfort modeling can range from high-fidelity computational fluid dynamics to simplified resistance-capacitance models and empirical control models. High-fidelity simulation provides detailed flow patterns but requires more parameters and computational resources. Since the goal of this work is a deployable prototype rather than a CFD-grade simulator, we adopt physics-inspired influence fields with interpretable parameters.

\subsection{Sparse Sensing and Spatial Interpolation}
When only sparse sensor observations are available, interpolation is a common baseline. IDW estimates an unknown value using distance-weighted nearby observations and has been widely used for spatial approximation \cite{shepard1968two}. However, pure interpolation ignores the location, type, orientation, and expected effect of appliances. This paper compares the proposed appliance-aware field model against IDW to show the value of incorporating equipment influence structure.

\subsection{Tool Interfaces for AI Systems}
Recent AI systems increasingly use external tools to retrieve context, execute code, or call domain-specific services. MCP provides a standardized interface through which AI clients can access tools and resources exposed by external systems \cite{mcp2025}. In this work, MCP is not treated as a protocol contribution. Instead, it is used as a system integration layer that makes the spatial digital twin callable by AI clients.
...
\begin{equation}
F_m(\mathbf{p},t) = B_m(\mathbf{p}) + \sum_j I_{j,m}(\mathbf{p},t) + C_m(\mathbf{p}),
\end{equation}
where $\mathbf{p}=(x,y,z)$, $B_m$ is the background field, $I_{j,m}$ is the influence of appliance $j$, and $C_m$ is a correction field fitted from sensor residuals.
...
\end{equation}
The prototype fits an affine correction:
\begin{equation}
C_m(x,y,z)=\alpha_{0,m}+\alpha_{1,m}x+\alpha_{2,m}y+\alpha_{3,m}z.
\end{equation}
This correction cannot represent high-frequency turbulence or complex local reflections, but it can correct overall bias and first-order spatial gradients using only eight corner measurements.
...
The local MCP server exposes tools for listing scenarios, running a scenario, ranking actions, sampling a point, comparing against IDW, learning appliance impacts, listing window scenarios, and running the full window matrix. These tools allow MCP-compatible clients to access the digital twin without directly importing Python modules.

\subsection{Interactive Web Demo}
The web demo provides a rotatable 3D canvas visualization. The user can rotate and zoom the estimated field, switch the metric using checkbox controls, and toggle appliances using checkboxes. The air conditioner is displayed as a wall bar, the window as a wall rectangle, and the light as a point marker. The interface also displays target-zone estimates, recommendations, baseline comparison, impact learning results, point sampling, and the 48-case window matrix.
...
\hline
idle & 28.87 & 67.50 & 90.00 & ac\_and\_light\\
ac\_only & 26.97 & 66.39 & 90.00 & turn\_on\_light\\
window\_only & 29.14 & 67.85 & 208.41 & ac\_and\_light\\
light\_only & 29.12 & 67.50 & 416.53 & turn\_on\_ac\\
all\_active & 27.46 & 66.66 & 443.38 & turn\_on\_ac\\
\hline
\end{tabular}
\end{table}

\subsection{Field Reconstruction Error}
Across the eight validation scenarios, the average mean absolute error (MAE) was 0.0626 for temperature, 0.1867 for humidity, and 3.5248 for illuminance. Illuminance has the largest numerical MAE because its scale is larger and because it is more sensitive to device position and directionality.
...
\hline
light\_only & Temp. & 0.0493 & 0.1592 & 69.03\%\\
light\_only & Hum. & 0.1839 & 0.5104 & 63.97\%\\
light\_only & Illum. & 5.4440 & 106.7814 & 94.90\%\\
ac\_only & Temp. & 0.0779 & 0.7613 & 89.77\%\\
ac\_only & Hum. & 0.1905 & 0.7245 & 73.71\%\\
ac\_only & Illum. & 1.8393 & 1.3876 & -32.55\%\\
\hline
\end{tabular}
\end{table}
...
The window matrix contains 48 cases: four times of day, three weather states, and four seasons. This matrix evaluates the window as an environmental variable rather than a simple binary switch. Table~\ref{tab:window} shows representative cases.
...
\hline
summer-sun-noon & 37.0 & 71.0 & 36000.0 & 223.90\\
winter-rain-night & 11.0 & 78.0 & 15.2 & 68.90\\
spring-cloud-morn. & 21.5 & 70.0 & 5005.0 & 90.39\\
\hline
\end{tabular}
\end{table}
